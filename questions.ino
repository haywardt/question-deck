...
<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Question Deck</title><style>*{margin:0;padding:0;box-sizing:border-box}body{background:#000;color:#fff;font-family:Arial,sans-serif;display:flex;justify-content:center;align-items:center;min-height:100vh;padding:20px}#counter{position:fixed;top:20px;right:20px;font-size:1.2rem;color:#a8dadc;z-index:100}#main{width:100%;max-width:600px;text-align:center}#question{font-size:3rem;font-weight:bold;min-height:200px;display:flex;align-items:center;justify-content:center;margin-bottom:40px;line-height:1.2;word-wrap:break-word}#question.listen{color:#87ceeb;font-size:2rem;font-weight:normal;font-style:italic}#timer{font-size:1.8rem;font-weight:bold;margin-top:30px;color:#ffd700}#pulse{position:fixed;width:30px;height:30px;border:2px solid #ff6b6b;border-radius:50%;transform:translate(-50%,-50%);opacity:0;pointer-events:none;box-shadow:0 0 30px rgba(255,107,107,0.8),0 0 60px rgba(255,107,107,0.5)}#pulse.active{animation:expand 120s linear forwards,pulseGlow 1.5s ease-in-out infinite;opacity:0.7}@keyframes expand{0%{width:30px;height:30px}100%{width:500px;height:500px}}@keyframes pulseGlow{0%,100%{box-shadow:0 0 30px rgba(255,107,107,0.5),0 0 60px rgba(255,107,107,0.3)}50%{box-shadow:0 0 40px rgba(255,107,107,0.9),0 0 70px rgba(255,107,107,0.6)}}@media (max-width:600px){#question{font-size:2rem;min-height:150px}#question.listen{font-size:1.5rem}#timer{font-size:1.5rem}}</style></head><body><div id="counter">60/0</div><div id="pulse"></div><div id="main"><div id="question">Waiting...</div><div id="timer"></div></div><script>let sessionId=localStorage.getItem('sessionId')||'';const lightListenMsgs=['Pay attention','Lean in close','Focus up','Listen closely','Get ready','Stay tuned','This is good','Pay it forward'];const deepListenMsgs=['This is profound','Listen closely','Pay attention','Lean in','What wisdom','This matters','Remember this','Let it sink in'];function pickListenMsg(isDeep){const msgs=isDeep?deepListenMsgs:lightListenMsgs;return msgs[Math.floor(Math.random()*msgs.length)]}let currentListenMsg='';let currentQuestion='';let prevRemaining=15000;let wakeLock=null;let inactivityTimer=null;let lastInteractionTime=Date.now();const INACTIVITY_THRESHOLD=15000;function setRandomPosition(){const pulse=document.getElementById('pulse');const left=Math.random()*80+10;const top=Math.random()*80+10;pulse.style.left=left+'%';pulse.style.top=top+'%'}function resetInactivityTimer(){clearTimeout(inactivityTimer);const pulse=document.getElementById('pulse');pulse.classList.remove('active');inactivityTimer=setTimeout(()=>{setRandomPosition();document.getElementById('pulse').classList.add('active')},INACTIVITY_THRESHOLD)}function handleUserInteraction(){resetInactivityTimer()}async function requestWakeLock(){try{if('wakeLock' in navigator){wakeLock=await navigator.wakeLock.request('screen');document.addEventListener('visibilitychange',()=>{if(document.hidden&&wakeLock){wakeLock.release();wakeLock=null}else if(!document.hidden&&!wakeLock){requestWakeLock()}})}}catch(err){console.log('Wake lock failed',err)}}function registerSession(){fetch('/api/register?id='+sessionId).then(r=>r.json()).then(d=>{sessionId=d.id;localStorage.setItem('sessionId',sessionId);updateState()}).catch(e=>console.error(e))}function updateState(){fetch('/api/questions?session='+sessionId).then(r=>r.json()).then(d=>{const qEl=document.getElementById('question');const remaining=Math.ceil(d.remaining/1000);document.getElementById('timer').innerText=remaining+'s';document.getElementById('counter').innerText='60/'+d.count;if(d.active){qEl.innerText=d.question;qEl.classList.remove('listen');currentQuestion=d.question}else{if(d.question!==currentQuestion){currentListenMsg=pickListenMsg(d.isDeep);currentQuestion=d.question}qEl.innerText=currentListenMsg;qEl.classList.add('listen')}prevRemaining=remaining}).catch(e=>console.error(e))}registerSession();requestWakeLock();resetInactivityTimer();setInterval(updateState,500);document.addEventListener('click',handleUserInteraction);document.addEventListener('touchstart',handleUserInteraction);</script></body></html>
)rawliteral";
